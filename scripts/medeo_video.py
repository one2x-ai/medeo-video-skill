...
def _load_config_from_file(env: Optional[str] = None) -> dict:
    ...
    # Multi-env format
    if "prd" in raw and isinstance(raw["prd"], dict):
        cfg = dict(raw["prd"])
        cfg.setdefault("env", "prd")
        return cfg
...
def _check_api_key(config: dict):
    ...
    if not config.get("apiKey"):
        get_key_url = ENV_DEFAULTS["prd"]["apiKeyUrl"]
        print(json.dumps({
...
def api_initiate_video_creation(config: dict, message_text: str,
    ...
    body: Dict[str, Any] = {
        "message": {
            "sender_id": f"openclaw_{uuid.uuid4().hex[:12]}",
            "content": [
                {
                    "text": {
                        "type": "text",
...
def cmd_config_init(args, config: dict):
    ...
    if args.api_key:
        ...
        raw["prd"]["apiKey"] = args.api_key

    # Set defaults
    defaults = ENV_DEFAULTS["prd"]
    raw["prd"].setdefault("baseUrl", defaults["baseUrl"])
    raw["prd"].setdefault("ossBaseUrl", defaults["ossBaseUrl"])
    raw["env"] = "prd"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Environment is always PRD — no --env flag needed
